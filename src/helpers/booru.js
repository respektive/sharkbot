...
async function gelbooru(channel, tags) {
    if (!channel.nsfw) tags.push("-rating:explicit", "-rating:questionable");
    ...
    if (posts.length == 0) return { errorMessage: `No posts found for tags ${tagString}` };

    const post = posts[0];
    let postUrl = post.postView;

    if (tags.length > 0) postUrl += "&tags=" + tags.join("+");
...
async function konachan(channel, tags) {
    if (!channel.nsfw) tags.push("rating:safe");
    let tagString = tags.join(", ");
    let tagArray = tags.concat(Array.from(blacklist).map((i) => "-" + i));
